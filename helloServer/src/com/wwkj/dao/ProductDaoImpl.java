...
public class ProductDaoImpl {
	 ...
	 * @return
	 */
	public Product getProductDesc(String productId) {
		String sql = "select * from product where id = ?;";
		Product p = null;
		try {
			p =  runner.query(sql, new BeanHandler<Product>(Product.class), productId);
	...
	public Product getProduct(String productId) {
		Product product = getProductDesc(productId);
		if (product != null) {
...
//				}
				//fillProperty(product);
			Product p = getProductDesc(productId);
			if(p!=null){
				map.put("response", "product");
				map.put("product", p);
			} else {
...
//		}
	 ...
	 *            ：指定的商品
	 */
	private void fillProperty(Product p) {
